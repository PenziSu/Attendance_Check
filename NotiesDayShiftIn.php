...
//===取得未打卡名單====================================
$vacation=array();//休假名單
$Missman = getDutyList($vacation,$compareList,"d");
...
//====執行打卡比對,發LINE通知的內容============
/*for ($i=0;$i<count($clockList);$i++){ $clockList[] = $clockList[$i][1];}
foreach ($dutyList as $item){
	if (in_array($item,$clockList1) !== true){
		//
		//$para=$item.' did not clock in/out';
